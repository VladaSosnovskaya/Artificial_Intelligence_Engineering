...
def missing_table(df: pd.DataFrame, min_missing_share = 0) -> pd.DataFrame:
    """
    ...
    result = result[result["missing_share"] >= min_missing_share]
    return result
...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    # флаг, сигнализирующий, что есть категориальные признаки с очень большим числом уникальных значений
    # т.е. если хотя бы в одном столбце количество уникальных признаков больше 80 процентов
    has_high_uniq_cat = any(not col.is_numeric and col.non_null > 0 and col.unique / summary.n_rows > 0.8 for col in summary.columns)
    flags["has_high_uniq_categoricals"] = has_high_uniq_cat

    # проверка, что идентификатор (например, user_id) уникален; при наличии дубликатов выставлять флаг
    # если есть id в названии столбца - значит должно быть уникально
    has_id_duplicates = any(col.unique < col.non_null and "id" in col.name.lower() and col.non_null > 0 for col in summary.columns)
    flags["has_id_duplicates"] = has_id_duplicates
