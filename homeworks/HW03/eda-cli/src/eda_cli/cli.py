...
def head(
    ...
    n: int = typer.Option(5, "--n", help="Сколько строк вывести (по умолчанию: 5)."),
...
def report(
    ...
    title: str = typer.Option("EDA-отчёт", help="Заголовок отчёта в файле report.md."),
    min_missing_share: float = typer.Option(0, help="Порог доли пропусков (0.0–1.0), выше которого колонка добавляется в missing.csv"),
    ...
    with md_path.open("w", encoding="utf-8") as f:
        ...
        f.write(f"- Есть категориальные признаки с >80% уникальных значений: **{quality_flags['has_high_uniq_categoricals']}**\n")
        f.write(f"- В id есть дубликаты: **{quality_flags['has_id_duplicates']}**\n\n")
        
        f.write(f" Указанные параметры: top-k категорий = {top_k_categories}, порог пропусков = {min_missing_share:.1%}\n\n")
