...
def test_has_high_cardinality_categoricals():
    df = pd.DataFrame({
        "user_id": range(100),
        "category": [f"cat_{i}" for i in range(90)] + ["cat_0"] * 10  # 90 процентов уникальных значений
    })
    summary = summarize_dataset(df)
    missing_df = missing_table(df)
    flags = compute_quality_flags(summary, missing_df)
    assert flags["has_high_uniq_categoricals"] is True

def test_has_id_duplicates():
    df = pd.DataFrame({
        "user_id": [1, 2, 2, 4, 5],  # дубликат 2
