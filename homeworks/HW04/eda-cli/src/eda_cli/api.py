...
METRICS = {
    "total_requests": 0,
    "total_latency_ms": 0.0,  # сумма всех задержек (в мс)
}
...
class QualityRequest(BaseModel):
    ...
    has_constant_columns: bool = Field(..., description="Есть ли в датасете хотя бы одна колонка с одинаковыми значениями: True или False")
    has_high_uniq_cat: bool = Field(..., description="Есть ли в датасете хотя бы одна колонка, в которой слишком много уникальных признаков: True или False")
    has_id_duplicates: bool = Field(..., description="Есть ли дубликаты в любом поле, содержащим 'id' в названии: True или False")
...
def quality(req: QualityRequest) -> QualityResponse:
    ...
    if req.has_constant_columns:
        score -= 0.1
    if req.has_high_uniq_cat:
        score -= 0.1
    if req.has_id_duplicates:
        score -= 0.05
    ...
    flags = {
        ...
        "has_constant_columns": req.has_constant_columns,
        "has_high_uniq_cat": req.has_high_uniq_cat,
        "has_id_duplicates": req.has_id_duplicates
    }
...
# ---------- /quality-flags-from-csv: на основе CSV файла возвращает полный набор флагов качества ----------


@app.post(
    "/quality-flags-from-csv",
    tags=["quality"],
    summary="Получение полного набора флагов качества по CSV-файлу с использованием EDA-ядра",
)
async def quality_flags_from_csv(file: UploadFile = File(...)):
    ...
    print(
        f"[quality_flags_from_csv] filename={file.filename!r} "
        f"flags={flags} "
        f"latency_ms={latency_ms:.1f} ms"
    )
    ...
    return {
        "flags": {
            ...
            "has_high_uniq_categoricals": flags["has_high_uniq_categoricals"],
            "has_id_duplicates": flags["has_id_duplicates"],
        }
    }


# ---------- /metrics - отслеживание метрик (можно в будущем использовать для визуализации) ----------
@app.get("/metrics", tags=["system"])
async def get_metrics():
    """
    Возвращает текущую статистику сервиса:
    - total_requests: общее число HTTP-запросов
    - avg_latency_ms: средняя задержка (по всем запросам)
    """
