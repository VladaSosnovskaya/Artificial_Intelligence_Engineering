...
| 2  | Endpoint `/predict` использует **реальную модель**, а не заглушку        |       ✅          | `src/service/inference.py/`, `configs/inference_config.yaml`             |
| 3  | Есть EDA и хотя бы один эксперимент с метриками                          |        ✅         | `notebooks/exp01_eda.ipynb`, `report.md`     |
| 4  | Есть baseline и улучшенная модель, есть **сравнение по метрикам**        |         ✅        | `notebooks/exp02_baseline_models.ipynb`, `notebooks/exp03_cnn.ipynb`, `notebooks/exp04_augs.ipynb`, `notebooks/exp05_metrics_analyse.ipynb`, `report.md` |
| 5  | Код не свален в один ноутбук: есть внятная структура в `src/`            |         ✅        | `src/data/`, `src/features/`, `src/models/`, `src/service/`, `src/utils` |
| 6  | Есть Dockerfile **или** понятный сценарий развёртывания без Docker       |         ✅        | `Dockerfile.cpu`, `Dockerfile.gpu`, шаги в `README.md`       |
